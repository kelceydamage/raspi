...
REQUIRES:       host [ip/hostname]
    ...
    def ship(self, header, envelope, n):
        ...
        try:
            ...
            self.r_log(message, '[FRONTEND] Shipping')
            self.backend.send_multipart(message)
        except Exception as e:
            printc('[ROUTER] (ship): {0}'.format(str(e)), COLOURS.RED)
...
REQUIRES:       message
        ...
        n = 1
        z = 0
        self.r_log(message, '[FRONTEND] Chunker')
    ...
    def assemble(self, message):
        self.r_log2(message, '[BACKEND] Assembler')
        try:
            meta = MetaFrame(0)
            meta.load(meta.deserialize(message[2]))
            ...
            if self.state[meta.get_serial()] == 0:
                ...
                self.r_log2(message, '[BACKEND] Shipping')
                self.frontend.send_multipart(message)
                self.buffer = []
                self.r_log2(self.state, '[BACKEND] State')
...
DESCRIPTION:    Main routing component [loop]
        ...
        while True:
            ...
            if socks.get(self.frontend) == zmq.POLLIN:
                ...
                self.r_log(message, '[FRONTEND] Router receive')
                if CHUNKING == True:
                    self.log(
                    ...
                    print('[FRONTEND] Sending to chunker')
                    self.chunk(message)
                    self.r_log(self.state, '[FRONTEND] State')
                else:
                    self.log('Forwarding', 'FRONTEND', message[2])
                    print('[FRONTEND] Forwarding raw')
                    self.backend.send_multipart(message)

            if socks.get(self.backend) == zmq.POLLIN:
                printc('[BACKEND]: Polling', COLOURS.GREEN)
                message = self.backend.recv_multipart()
                self.r_log2(message, '[BACKEND] Router receive')
                self.log('Forwarding', 'BACKEND', message[2])
                if CHUNKING == True:
                    printc('[BACKEND] Sending to assembler', COLOURS.GREEN)
